...
const charCode = (arr) => {
  const toChar = arr.map((val, idx) => arr[idx].charCodeAt());
    return toChar;
}
...
const evenOdd = (arr) => {
  const nums = arr.map((val, idx) =>{
    if(typeof val !== 'number'){
      arr.splice(idx, 1, 'N/A')
    } else if(val % 2 === 0){
      arr.splice(idx, 1, 'even')
    } else if(val % 2 !== 0){
      arr.splice(idx, 1, 'odd')
    }
  });
    console.log(nums);
    return arr;
...
const extractAbilities = (arr) => {
  const snorlaxAbility = arr.map ((val, idx)=> val.ability.name);
    return snorlaxAbility;
